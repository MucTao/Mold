...
@Composable
fun <T> MoldTreeCheckView(
    sourceList: List<MoldTreeDropdownOption<T>>,
    checkList: List<MoldTreeDropdownOption<T>>,
    onCheckChange: (List<MoldTreeDropdownOption<T>>) -> Unit,
) {
    Row(Modifier.width(600.dp), horizontalArrangement = Arrangement.spacedBy(Dimensions.paddingXSmall)) {
        ...
        val filteredTreeItems by remember(sourceList, checkList, filter, expandedNodes) {
            derivedStateOf {
                ...
                fun traverse(nodes: List<MoldTreeDropdownOption<T>>, level: Int) {
                    for (node in nodes) {
                ...
                ListItem(
                    ...
                    trailingContent = {
                        ...
                        IconButton(
                            onClick = {
                                val checkedValues = checkList.mapTo(HashSet()) { it.id }
                                val leavesToAdd = mutableListOf<MoldTreeDropdownOption<T>>()
...
private fun <T> isAllChildrenChecked(
    children: List<MoldTreeDropdownOption<T>>,
    checkedValues: Set<String>
): Boolean {
