 ...
 */
@Immutable
data class MucDropdownOption<T>(
    val value: T,
    val label: String,
...
// 树形下拉选项模型
data class MucTreeDropdownOption<T>(
    val label: String,
    val value: T,
    val children: List<MucTreeDropdownOption<T>> = emptyList(),
 ...
 * 递归过滤，直到根节点数量不为 1
 */
tailrec fun <T> List<MucTreeDropdownOption<T>>.filterSingleRootRecursive(): List<MucTreeDropdownOption<T>> {
 ...
 */
fun <T> List<MucTreeDropdownOption<T>>.dfsDistinct(visited: MutableSet<String> = mutableSetOf()): List<MucTreeDropdownOption<T>> {
    return this
...
internal enum class MucTextFieldStyle {
    FILLED,
    OUTLINED,
    PLAIN,
}
...
@Immutable
internal data class MucTextFieldResolvedColors(
