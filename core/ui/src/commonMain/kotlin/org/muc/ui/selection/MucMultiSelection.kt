 ...
 */
@Immutable
data class MucMultiSelectionState<T>(
 ...
 */
fun <T> MucMultiSelectionState<T>.retainVisible(
    visibleIds: Collection<T>,
): MucMultiSelectionState<T> {
...
fun <T> MucMultiSelectionState<T>.clearSelection(): MucMultiSelectionState<T> =
    if (selectedIds.isEmpty() && anchorId == null) this else MucMultiSelectionState()

/**
 * Выделить все строки из [visibleIds].
 */
fun <T> MucMultiSelectionState<T>.selectAll(
    visibleIds: List<T>,
): MucMultiSelectionState<T> {
    if (visibleIds.isEmpty()) return clearSelection()
    return MucMultiSelectionState(
 ...
 */
fun <T> MucMultiSelectionState<T>.onItemSelectionRequested(
    itemId: T,
    visibleIds: List<T>,
