    ...
    createClientPlugin("CurlLogger") {
        ...
        onResponse { response ->
            ...
            if (canLog || onServerTime != null) {
                ...
                if (enableLog()) {
                    println("onResponse url:${response.request.url}\n status:${response.status}\n body :${response.bodyAsText()}")
                    attrs.getOrNull(historyIdKey)?.let { historyId ->
                ...
                if (onServerTime != null) {
                    if (durationMs < minResponseTime) {
                        val dateHeader = response.headers["Date"]
                        if (dateHeader.isNullOrBlank()) return@onResponse
                        onServerTime(dateHeader, durationMs)
                        minResponseTime = durationMs
                    }
                }
            }
        }
